...
export default async function GithubAuth(ctx: Context): Promise<AuthUserInfo> {
    ...
    let accessTokenResult = await githubRes.json();
    let accessToken = accessTokenResult.access_token;
    let tokenType = accessTokenResult.token_type;
