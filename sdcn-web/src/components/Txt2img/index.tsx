...
import { ModelFormGroup, LoraFormGroup, SamplingFormGroup } from 'components/SettingsFormGroup';
  ...
  return (
    <div className={cx('flex flex-col items-start gap-6')}>
      ...
      <div className={cx('flex flex-col w-full items-start gap-6')}>
        <Form ref={props.promptRef} name='txt2imgPromptForm' style={{width: '100%'}}>
          <Form.Item name='prompt' className={cx('self-stretch')} style={{marginBottom: '0px'}}>
            <TextArea  size='large' rows={6} placeholder="Enter prompts here"
                className={cx('text-base leading-6 px-4 py-2')}
              />
          </Form.Item>
        </Form>
        <Button type="primary" ref={props.buttonRef} size='large'>Generate</Button>
      </div>
    </div> 
  );
} 
...
function SettingsArea(props:any) {
  return (
    <div className={cx('flex flex-col w-80 gap-6 bg-green-000')}>
      ...
      <Form ref={props.settingsRef} name='txt2imgSettingsForm' layout="vertical">
        ...
        <Form.Item label="Negative Prompts" name="negative_prompt">
          <TextArea size='large' rows={4} placeholder="Negative Prompts"
              className={cx('self-stretch text-base leading-6 px-4 py-2')} />
        </Form.Item>

        <SamplingFormGroup methodName='sampling_method' stepsName='steps' seedName='seed' />
    </Form>
    </div>
  );
}
...
const Txt2img = () => {
  ...
  useEffect(()=>{
    if(GenerateButtonRef.current) // Bind click event
  ...
  return (
    ...
    <div className={cx(styles.wrap + ' flex flex-col md:flex-row w-full bg-yellow-000 gap-24 mt-8')}>
      <div className={cx('flex flex-col flex-1 bg-red-000')}>
        <InputAndGenerateArea buttonRef={GenerateButtonRef} promptRef={PromptFormRef} /> 
